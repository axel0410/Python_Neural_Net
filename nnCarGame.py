...
size = width,height = 848, 480 #Size to use when creating pygame window
...
white_small_car = pygame.transform.scale(white_small_car, (int(width * 0.010625), int(height * 0.03888)))
white_big_car = pygame.transform.scale(white_big_car, (int(width * 0.02), int(height * 0.0611)))
green_small_car = pygame.transform.scale(green_small_car, (int(width * 0.010625), int(height * 0.03888)))
green_big_car = pygame.transform.scale(green_big_car, (int(width * 0.02), int(height * 0.0611)))

bg = pygame.image.load('bg7.png')
bg4 = pygame.image.load('bg4.png')
bg4 = pygame.transform.scale(bg4, size)
...
class AINeuralNetwork:
    def __init__(self, sizes, id):
        ...
        self.input = np.zeros((sizes[0], 1), dtype=float)#np.array([[0], [0], [0], [0], [0], [0]])
        self.output = np.zeros((sizes[2], 1), dtype=float) #MAYBE MEANT TO BE INTS???
    ...
    def feedforward(self, inputArray):
        ...
        for b, w in zip(self.biases, self.weights):
            self.input = self.sigmoid(np.dot(w, self.input)+b)

        self.output = self.input
        return self.output
    
    def __copyweightsfrom(self, child):
        if(child.sizes !=  self.sizes):
            print("Cannot copy from different neural network sizes")
            exit()
        else:
            self.weights = child.weights


    def __copybiasesfrom(self, child):
        if(child.sizes !=  self.sizes):
            print("Cannot copy from different neural network sizes")
            exit()
        else:
            self.biases = child.biases


    @staticmethod
    def mutateOneWeightGene(child1):
        sizenn = len(child1.sizes)         

        #child1.__copyweightsfrom(parent1)
    ...
    @staticmethod
    def mutateOneBiasesGene(child1):
        sizenn = len(child1.sizes)        
    ...
    @staticmethod
    def crossovergenome(genome1, genome2):
        newgenome = []
   
        for i in range(0, len(genome1), 2):
            newgenome.append(genome1[i])
            if(i+1 < len(genome2)):
                newgenome.append(genome2[i+1])

        return newgenome


    @staticmethod
    def uniformCrossOverWeights(parent1, parent2, child1): #Given two parent car objects, it modifies the children car objects weights
        sizenn = len(parent1.sizes) #3 si car1=Car([2, 4, 3])
                    
        genome1 = [] #This will be a list containing all weights of child1
        genome2 = [] #This will be a list containing all weights of child2
            
        for i in range(sizenn-1): #i=0,1
            for j in range(parent1.sizes[i]*parent1.sizes[i+1]):
                genome1.append(parent1.weights[i].item(j))
                
        for i in range(sizenn-1): #i=0,1
            for j in range(parent2.sizes[i]*parent2.sizes[i+1]):
                genome2.append(parent2.weights[i].item(j))

        #Crossover weights          
        newgenome = AINeuralNetwork.crossovergenome(genome1, genome2)
        ...
        for i in range(sizenn-1):
            for j in range(child1.sizes[i+1]):
                for k in range(child1.sizes[i]):
                    child1.weights[i][j][k] = newgenome[count]
                    count += 1
                   
        return 

    @staticmethod
    def uniformCrossOverBiases(parent1, parent2, child1): #Given two parent car objects, it modifies the children car objects biases
        ...
        for i in range(sizenn-1):
            for j in range(parent1.sizes[i+1]):
                genome1.append(parent1.biases[i].item(j))
                
        for i in range(sizenn-1):
            for j in range(parent2.sizes[i+1]):
                genome2.append(parent2.biases[i].item(j))
        ...
        for i in range(sizenn-1):
            for j in range(child1.sizes[i+1]):
                child1.biases[i][j] = newgenome[count]
                count += 1
...
class Car:
  def __init__(self):
    ...
    self.x = 0.133 * width - 40
    self.y = 0.3 * height + 40
    self.center = self.x, self.y
    #Height and width of the car
    self.height = height * 0.0388 #35 #45
    self.width = width * 0.010625 #17 #25
  ...
  def update(self): 
    ...
    move_unit = 10
    move_unitS = -1
    #I think this is creating the 'seeing' lines of where cars could colide
    self.c1 = move((self.x,self.y),self.angle,move_unit)
    while bg4.get_at( (int (self.c1[0]) , int (self.c1[1]) ) ).a!=0: #while alpha is not 0
        self.c1 = move((self.c1[0],self.c1[1]),self.angle,move_unit)   
    while bg4.get_at((int(self.c1[0]),int(self.c1[1]))).a==0: #while alpha is 0
        self.c1 = move((self.c1[0],self.c1[1]),self.angle,move_unitS)

    self.c2 = move((self.x,self.y),self.angle+45,move_unit)
    while bg4.get_at((int(self.c2[0]),int(self.c2[1]))).a!=0:
        self.c2 = move((self.c2[0],self.c2[1]),self.angle+45,move_unit)
    while bg4.get_at((int(self.c2[0]),int(self.c2[1]))).a==0:
        self.c2 = move((self.c2[0],self.c2[1]),self.angle+45,move_unitS)

    self.c3 = move((self.x,self.y),self.angle-45,move_unit)
    while bg4.get_at((int(self.c3[0]),int(self.c3[1]))).a!=0:
        self.c3 = move((self.c3[0],self.c3[1]),self.angle-45,move_unit)
    while bg4.get_at((int(self.c3[0]),int(self.c3[1]))).a==0:
        self.c3 = move((self.c3[0],self.c3[1]),self.angle-45,move_unitS)
        
    self.c4 = move((self.x,self.y),self.angle+90,move_unit)
    while bg4.get_at((int(self.c4[0]),int(self.c4[1]))).a!=0:
        self.c4 = move((self.c4[0],self.c4[1]),self.angle+90,move_unit)
    while bg4.get_at((int(self.c4[0]),int(self.c4[1]))).a==0:
        self.c4 = move((self.c4[0],self.c4[1]),self.angle+90,move_unitS)
        
    self.c5 = move((self.x,self.y),self.angle-90,move_unit)
    while bg4.get_at((int(self.c5[0]),int(self.c5[1]))).a!=0:
        self.c5 = move((self.c5[0],self.c5[1]),self.angle-90,move_unit)
    while bg4.get_at((int(self.c5[0]),int(self.c5[1]))).a==0:
        self.c5 = move((self.c5[0],self.c5[1]),self.angle-90,move_unitS)
...
class AICar(Car):
    def __init__(self, sizes, id):
        super().__init__()
        self.neural_net = AINeuralNetwork(sizes, id)

    
    def FeedInputsForward(self):
        inputArray = np.array([[self.d1], [self.d2], [self.d3], [self.d4], [self.d5], [self.velocity]])
        self.neural_net.feedforward(inputArray)
    ...
    #   else:
    #        self.set_accel(0)      
        #if self.neural_net.output.item(1) > 0.5: #Brake
        #    self.add_accel(-0.6)     
        if self.neural_net.output.item(0) > 0.5: #Turn right
            self.rotate(-5)    
        if self.neural_net.output.item(1) > 0.5: #Turn left
            self.rotate(5) 
        return
...
def displayTexts():  
    infotextX = width*0.0125
    infotextY = height*0.75
    ...
    infotext9 = font.render('Frames: ' + str(frames), True, white)
...
auxcar = AICar([inputLayer, hiddenLayer, outputLayer], -1)

for i in range(num_of_nnCars):
	nnCars.append(AICar([inputLayer, hiddenLayer, outputLayer], i))
...
def redrawGameWindow(): #Called on very frame   
    ...
    gameDisplay.blit(pygame.transform.scale(bg, size), (0, 0)) 
            ...
            if event.key == ord ( "b" ):
                if (len(selectedCars) == 2):
                    ...
                    for i in range(num_of_nnCars-2):
                        AINeuralNetwork.uniformCrossOverWeights(selectedCars[0].neural_net, selectedCars[1].neural_net, nnCars[i].neural_net)
                        AINeuralNetwork.uniformCrossOverBiases(selectedCars[0].neural_net, selectedCars[1].neural_net, nnCars[i].neural_net)
                    ...
                    for i in range(num_of_nnCars-2):
                        for j in range(mutationRate):
                            AINeuralNetwork.mutateOneWeightGene(nnCars[i].neural_net)

                            AINeuralNetwork.mutateOneBiasesGene(nnCars[i].neural_net)
